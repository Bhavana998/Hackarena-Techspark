...
# ============================================================================
    ...
    with col3:
        status_filter = st.multiselect("Status", ["Approved", "Flagged", "Under Review"])
    
    # Sample data
    audit_df = pd.DataFrame({
        "Timestamp": pd.date_range(start='2024-01-01', periods=50, freq='H'),
        "Submission ID": [f"SUB-{i:04d}" for i in range(1000, 1050)],
        "Company": np.random.choice(list(COMPANY_COLORS.keys()), 50),
        "Level": np.random.choice(["IC3", "IC4", "IC5"], 50),
        "Total Comp": np.random.randint(150000, 500000, 50),
        "Quality Score": np.random.randint(65, 100, 50),
        "Status": np.random.choice(["Approved", "Flagged", "Under Review"], 50, p=[0.7, 0.2, 0.1])
    })
    ...
    st.download_button("📥 Export to CSV", csv, "audit_data.csv", "text/csv", use_container_width=True)
